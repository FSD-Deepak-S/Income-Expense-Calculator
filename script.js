...
const add_expense_btn = document.getElementById("add_expense");
let totalIncome;
let totalExpense;
let totalBalance;
...
const totalExp = () => {
  ...
  if (totalIncome === 0 || totalBalance === 0) {
    add_expense_btn.classList.add("hidden");
  } else {
    add_expense_btn.classList.remove("hidden");
  }
};
...
const formValidation = () => {
  if (description.value === "" || amount.value === "") {
    ...
    if (type.value === "Expense") {
      if (totalIncome < totalExpense + Number(amount.value)) {
        descError.innerHTML = "Expense is Higher than Income";
        descError.classList.add("text-red-500", "bg-white", "p-2");
        setTimeout(() => {
          descError.innerHTML = "";
          descError.classList.remove("text-red-500", "bg-white", "p-2");
        }, 3000);
      } else {
        descError.innerHTML = `${type.value} Saved Successfully`;
  ...
  if (tfilter === "all") {
    data.map((ele, y) => {
      transaction.innerHTML += `<tr id=${y} class="border h-10">
                  ...
                  <td class="">₹ ${parseInt(ele.amount).toLocaleString(
                    "en-IN"
                  )}</td>
                  <td class="">
...
const edittrans = (e) => {
  ...
  amount.value = Number(
    result.children[2].innerHTML.split(" ")[1].replace(/[^0-9.-]+/g, "")
  );
